...
export function MemberHomePage() {
  ...
  const { data: plans, isLoading: plansLoading } = useQuery({
    queryKey: ["plans"],
    queryFn: fetchPlans,
    enabled: !!data && (!data.membership || data.membership.status !== "ACTIVE") && !data.pendingPlanRequest,
  });
        ...
        {pendingPlanRequest ? (
          <Card className="neon-border overflow-hidden">
            ...
            <CardContent className="space-y-6">
              ...
              <p className="text-sm text-brand-muted px-4 leading-relaxed">
                Your membership request is currently pending admin approval. You will have access once approved.
              </p>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-brand-muted text-sm mt-2">Your membership is inactive. Choose a plan to continue.</p>
            </div>
            {plansLoading ? (
              ...
              <div className="space-y-4">
                {plans?.filter(p => p.isActive).map(plan => (
                  <Card key={plan.id} className="border border-white/10 bg-black/40 backdrop-blur text-left overflow-hidden">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div>
                        <p className="text-lg font-bold text-white">{plan.name}</p>
                        <p className="text-sm text-brand-muted">₹{(plan.priceCents / 100).toFixed(2)} / {plan.durationDays} days</p>
                      </div>
                      <div>
                        {isOnlinePaymentsEnabled ? (
                          <Button 
                            ...
                            onClick={() => rzpMutation.mutate({ planId: plan.id })}
                          >
                            Pay Online
                          </Button>
                        ) : (
                          <Button 
                            ...
                            onClick={() => requestMutation.mutate({ planId: plan.id })}
                          >
                            Request Admin
                          </Button>
                        )}
