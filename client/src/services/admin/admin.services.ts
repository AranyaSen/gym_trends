...
export async function fetchDashboard(): Promise<DashboardStats> {
  const { data } = await apiClient.get<ApiResponse<FetchDashboardResponse>>(
...
export async function fetchPlans(): Promise<PlanRow[]> {
  const { data } = await apiClient.get<ApiResponse<FetchPlansResponse>>(
    API_ROUTES.PLANS.LIST,
  );
  return data.data.plans;
}

export async function createPlan(body: CreatePlanPayload): Promise<PlanRow> {
  const { data } = await apiClient.post<ApiResponse<CreatePlanResponse>>(
    API_ROUTES.PLANS.CREATE,
    body,
  );
...
export async function assignMembership(
  body: AssignMembershipPayload,
): Promise<void> {
  await apiClient.post<ApiResponse<unknown>>(
    API_ROUTES.MEMBERSHIPS.ASSIGN,
    body,
  );
}

export async function renewMembership(
  body: RenewMembershipPayload,
): Promise<void> {
  await apiClient.post<ApiResponse<unknown>>(
    API_ROUTES.MEMBERSHIPS.RENEW,
    body,
  );
}

export async function switchMembership(
  body: SwitchMembershipPayload,
): Promise<void> {
  await apiClient.post<ApiResponse<unknown>>(
    API_ROUTES.MEMBERSHIPS.SWITCH,
    body,
  );
}

export async function fetchTrainers(): Promise<TrainerRow[]> {
  const { data } = await apiClient.get<ApiResponse<FetchTrainersResponse>>(
    API_ROUTES.TRAINERS.LIST,
  );
  return data.data.trainers;
}

export async function linkTrainerMember(
  body: LinkTrainerMemberPayload,
): Promise<void> {
  await apiClient.post<ApiResponse<unknown>>(API_ROUTES.TRAINERS.LINK, body);
}

export async function unlinkTrainerMember(
  trainerId: string,
  memberId: string,
): Promise<void> {
  await apiClient.delete<ApiResponse<unknown>>(
    API_ROUTES.TRAINERS.UNLINK(trainerId, memberId),
  );
}
...
export async function fetchAttendance(
  ...
  const { data } = await apiClient.get<ApiResponse<FetchAttendanceResponse>>(
...
export async function createManualAttendance(
  body: CreateManualAttendancePayload,
): Promise<void> {
  await apiClient.post<ApiResponse<unknown>>(
    API_ROUTES.ATTENDANCE.MANUAL,
    body,
  );
}
...
export async function fetchAuditLogs(
  ...
  const { data } = await apiClient.get<ApiResponse<FetchAuditLogsResponse>>(
    API_ROUTES.AUDIT_LOGS.LIST,
    { params },
  );
  return data.data;
}
